...
void Renderer::DrawEnemies(const std::vector<Enemy>& enemies, int Sta_PosX)
{
    for (auto& e : enemies)
        DrawGraph(e.Pos.X + Sta_PosX, e.Pos.Y, Pic.Enemy1, TRUE);
}

void Renderer::DrawUI(const Stage& sta)
{
...
void Renderer::DrawStage(const Stage& stage, int& Sta_PosX)
{
    stage.Out(&Sta_PosX);
}